...
const deleteCareerSubjectWebSocket = async (idUser: string, body: any) => {
  ...
  if (socketId !== null) {
    require("../../../server")
      .io.to(socketId)
      .emit("deleteCareerSubject", body);
  }
};

export const deleteCareerSubjectController = async (
  req: Request,
  res: Response
) => {
  const { idCareer, idSubject, idAdmin } = req.body;
  ...
  try {
    ...
    await deleteCareerSubjectWebSocket(idAdmin, req.body);

    res
      .status(200)
      .send("Action completed: A relation career-subject has been deleted");
  } catch (error) {
    console.log(error);
